...
//                    current = path.join(base, require);
    ...
    /**
     * find the final path of a module
     * @param base
     * @param require
     * @return {String}
     */
    var findModule = function(require){
        var modulePath = '',
            base = config.base;
    ...
    var comboFiles = function(target, files, cfg){
        ...
        commentTpl.push('*/\r\n');
        var combineFile = path.join(cfg.output, path.relative(target.base, target.file)).replace('.js', '.combine.js');
        ...
        for (var idx in files) {
            ...
            var relativefile = path.relative(target.base, f).replace(/\\/g, '\/'),
        ...
        //print console
        console.info('%s ===> %s', path.relative(target.base, target.file), combineFile);
    };
    /**
     * 打包单个文件
     * @param target
    ...
    var buildOnce = function(target, cfg) {
        var caches = analyzeDepends(target.file, target.base);
        caches = noDuplicate(caches);

        if(debug) {
            console.log('%s has %s depends to combine!\r\n', target.file, caches.length);
        }

        //add self
        caches.push(target.file);
...
//                cfg.base = path.resolve(cfg.base);
            ...
            for(var i = 0; i < cfg.base.length; i++){
                var target = path.join(cfg.base[i], cfg.target);
                console.log(target);
                //处理目录
                if(path.existsSync(target)){
                    if(fs.statSync(target).isDirectory()) {
                        ...
                        for (var idx in targets) {
                            if(path.extname(targets[idx])==='.js' && !isExcludeFile(targets[idx])) {
                                buildOnce({
                                    file: path.join(target, targets[idx]),
                                    base: cfg.base[i]
                                }, cfg);
                            }
                        }
